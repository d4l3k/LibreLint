...
class WebSync < Sinatra::Base
  ...
  before do
    ...
    if request.xhr? and not request.path_info.match %r{^/assets/}
      ...
      if bits.length < 2 or not (                    request.post? and path.match %r{^/#{doc}/upload$} or
  ...
  get '/new/:group' do
    ...
    doc = WSFile.create(            name: "Unnamed #{group.name}",
      body: { body: [] },
  ...
  post '/upload' do
    ...
    if params["convert"]
      ...
      if content != nil
        ...
        doc = WSFile.create(                    name: filename,
  ...
  get // do
    ...
    @javascripts = [            #'/assets/bundle-edit.js'
        ]
